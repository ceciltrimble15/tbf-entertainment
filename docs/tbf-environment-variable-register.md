...
| `AIRTABLE_BASE_ID` | TBF Entertainment | Airtable | Identifies the **TBF Entertainment Operations** base (`app6TQ7oSVSJADxC4`) | Production, Preview | operator | Non-secret; update only if the base changes | ☐ pending |
| `AIRTABLE_SUBMISSIONS_TABLE` | TBF Entertainment | Airtable | Table name — value must be `Website Submissions` | Production, Preview | operator | Non-secret; update only if the table is renamed | ☐ pending |
